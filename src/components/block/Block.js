...
function Block() {
  ...
  const [checkFreedom, setCheckFreedom] = useState(objectFreedom);

  const handleCheckFreedom = (number) => {
    switch (Number(number)) {
      case 1:
        setCheckFreedom({ ...objectFreedom, one: true });
        break;
      case 2:
        setCheckFreedom({ ...objectFreedom, two: true, one:false });
        break;
      case 3:
        setCheckFreedom({ ...objectFreedom, three: true, one:false });
        break;
      case 4:
        setCheckFreedom({ ...objectFreedom, four: true, one:false });
        break;
      case 0:
        ...
        <div className="block__proxy">
          <div className="block__proxy-top">
            ...
            <div className="block__proxy-top-content">
              ...
              <div className="block__proxy-top-content-text">
                ...
                network, there’s always a perfect proxy plan for every business
              ...
              <div className="block__proxy-top-content-btn">
                <button className="block__proxy-top-content-btn-black">
                  ...
                  >
                    ...
                    <FontAwesomeIcon
                      icon={faChevronRight}
                      className="block__proxy-top-content-btn-black-icon"
                    />
                  </Link>
                </button>
              </div>
          ...
          <div className="block__proxy-bot">
            <div className="block__proxy-bot-content">
              <h1>Looking For Full Scraping Freedom?</h1>

              <div className="block__proxy-bot-content-list">
                <div className={`block__proxy-bot-content-list-item ${checkFreedom.one ? "border-freedom" : ""}`}>
                  <h4
                    onClick={() => handleCheckFreedom(1)}
                  ...
                  >
                    <p>{`With 9Proxy, you'll blend in with the crowd like a chameleon. Our residential proxies come from real ISPs and home users, making sure you never get detected or blocked. No more vague geolocation data – with 9Proxy, accuracy is always on point.`}</p>
                  </div>
                </div>

                <div className={`block__proxy-bot-content-list-item ${checkFreedom.two ? "border-freedom" : ""}`}>
                  <h4 onClick={() => handleCheckFreedom(2)} className={checkFreedom.two ? "color-freedom" : ""}>
                    Unlimited concurrent sessions
                  </h4>
                  <div
                    className={`block__proxy-bot-content-list-item-text ${
                      checkFreedom.two ? "showed-height44" : "hidden"
                    }`}
                  >
                    <p>
                      There is no limit to the number of uses or invocation
                      frequencies of the proxies. It means you can work without
                      any interruptions or restrictions.
                    </p>
                  </div>
                </div>

                <div className={`block__proxy-bot-content-list-item ${checkFreedom.three ? "border-freedom" : ""}`}>
                  <h4 onClick={() => handleCheckFreedom(3)} className={checkFreedom.three ? "color-freedom" : ""}>
                    Hassle-free integration for a smooth start
                  </h4>
                  <div
                    className={`block__proxy-bot-content-list-item-text ${
                      checkFreedom.three ? "showed-height44" : "hidden"
                    }`}
                  >
                    <p>
                      We offer a simple and straightforward setup process that
                      allows you to configure your proxy settings and choose
                      your desired location with ease.
                    </p>
                  </div>
                </div>

                <div className={`block__proxy-bot-content-list-item ${checkFreedom.four ? "border-freedom" : ""}`}>
                  <h4 onClick={() => handleCheckFreedom(4)} className={checkFreedom.four ? "color-freedom" : ""}>
                    Get More Done, for Less
                  </h4>
                  <div
                    className={`block__proxy-bot-content-list-item-text ${
                      checkFreedom.four ? "showed-height44" : "hidden"
                    }`}
                  >
                    <p>
                      By providing top-notch residential proxy services at an
                      affordable price, 9Proxy offers unbeatable value, making
                      it the most cost-effective option on the market.
                    </p>
                  </div>
                </div>
              </div>
